...
type SimPhase = 'idle' | 'baseline' | 'scenario' | 'done'
...
interface Props {
  ...
  running: boolean
  phase?: SimPhase
  beforeDecision?: DecisionOut | null
  afterDecision?: DecisionOut | null
}

function ComparisonView({
  before, after, onEvidenceClick,
}: {
  before: DecisionOut
  after: DecisionOut
  onEvidenceClick: (side: 'before' | 'after') => void
}) {
  const confidenceDelta = after.confidence - before.confidence
  const allPredictionKeys = new Set([...Object.keys(before.prediction), ...Object.keys(after.prediction)])
  const changedPredictionKeys = new Set(
    Array.from(allPredictionKeys).filter(k => before.prediction[k] !== after.prediction[k])
  )
  const newRecommendations = new Set(
    after.recommendations.filter(r => !before.recommendations.includes(r))
  )

  const columns: { label: string; decision: DecisionOut; side: 'before' | 'after' }[] = [
    { label: 'Trước (Before)', decision: before, side: 'before' },
    { label: 'Sau (After)', decision: after, side: 'after' },
  ]
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between bg-slate-950 rounded-xl p-3 border border-slate-800">
        <div>
          ...
          <p className="text-sm text-slate-300 flex items-center gap-1.5">
            <span>{`${Math.round(before.confidence)}%`}</span>
            <span className="text-slate-600">→</span>
            <span className={confidenceDelta < 0 ? 'text-rose-400 font-bold' : confidenceDelta > 0 ? 'text-emerald-400 font-bold' : 'text-slate-300 font-bold'}>
              {`${Math.round(after.confidence)}%`}
            </span>
          </p>
        </div>
        {confidenceDelta !== 0 && (
          <span className={`text-xs font-mono ${confidenceDelta < 0 ? 'text-rose-400' : 'text-emerald-400'}`}>
      ...
      <div className="grid grid-cols-2 gap-3">
        {columns.map(({ label, decision, side }) => (
          <div key={side} className="bg-slate-950 rounded-xl p-3 border border-slate-800 space-y-2">
              ...
              <div className="space-y-1">
                {Object.entries(decision.prediction).map(([k, v]) => (
                  <div key={k} className="text-xs flex justify-between">
                    ...
                    <span className={changedPredictionKeys.has(k) ? 'text-amber-400 font-semibold' : 'text-slate-400'}>
                      {v}
                    </span>
                  </div>
                ))}
            ...
            {decision.recommendations.length > 0 && (
              <ul className="space-y-1">
                {decision.recommendations.map((r, i) => {
                  const isNew = side === 'after' && newRecommendations.has(r)
                  const evidence = decision.evidence ?? []
                  return (
                    <li
                      key={i}
                      onClick={evidence.length > 0 ? () => onEvidenceClick(side) : undefined}
                      className={`text-xs flex gap-1.5 ${evidence.length > 0 ? 'cursor-pointer hover:text-slate-100' : ''} ${
                        isNew ? 'text-amber-300 font-medium' : 'text-slate-300'
                      }`}
                    >
                      <span className="text-blue-400 shrink-0">▸</span>
                      <span>{r}{isNew ? ' (mới)' : ''}</span>
                    </li>
                  )
                })}
              </ul>
...
export default function SimulatorModal({
  districtName, onRun, onClose, events, running,
  phase = 'idle', beforeDecision = null, afterDecision = null,
}: Props) {
  const [selected, setSelected] = useState<string | null>(null)
  const [launched, setLaunched] = useState(false)
  const [evidenceSide, setEvidenceSide] = useState<'before' | 'after' | null>(null)
  ...
  const progressLabel =
    phase === 'baseline' ? 'Bước 1/2: Phân tích hiện trạng (Before)' :
    phase === 'scenario' ? 'Bước 2/2: Mô phỏng kịch bản (After)' :
    'Pipeline đang chạy'
  ...
  return (
    <>
      <div data-testid="simulator-modal" className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
        <div className="w-full max-w-2xl bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="p-6">
            ...
            ) : phase === 'done' && beforeDecision && afterDecision ? (
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <p className="text-green-400 text-sm font-semibold">✓ So sánh Before/After</p>
                  <button
                ...
                <ComparisonView before={beforeDecision} after={afterDecision} onEvidenceClick={setEvidenceSide} />
              </div>
            ) : (
              <>
                {/* Live pipeline progress */}
                <div className="space-y-2 mb-4">
                  <p className="text-xs text-slate-500 uppercase tracking-wider mb-3">{progressLabel}</p>
      ...
      {evidenceSide && (
        <EvidenceModal
          evidence={(evidenceSide === 'before' ? beforeDecision : afterDecision)?.evidence ?? []}
          onClose={() => setEvidenceSide(null)}
        />
      )}
    </>
  )
